...
effective_health = (2405 + HP) * (1 + ((AR*w_ad) / 100) + ((MR*w_ap) / 100))
...
CPV = (DPS_C/262) * ((effective_health / 6687) + (e_value / 6687))
    ...
    for t in range(iterations):
        ...
        for slot in range(5):  
            ...
            for item, stats in league_items.items():
                ...
                normalized_stats = {
                    stat: math.log(simulated_stats[stat] + 1) / math.log(max_value + 1) if max_value > 0 else 0
...
weights = update_weights(variables, weights, initial_stats, step_size=1)
build, score, final_weights = best_build(league_items, initial_stats, max_values, weights, variables, iterations=5, step_size=0.5)
print(build, score)